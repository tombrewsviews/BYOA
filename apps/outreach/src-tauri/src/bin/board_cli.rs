...
/// attributing every write to `actor` (the configured `OUTREACH_ACTOR`, or
/// "local"). Kept separate from `main` so it's unit-testable without
/// spawning a process or touching stdio.
pub fn dispatch(db: &mut Db, actor: &str, verb: &str, args: &Value) -> Result<Value, String> {
    ...
    match verb {
        ...
        "addLead" => {
            ...
            match board::add_lead(db, &name, org.as_deref(), &stage, actor) {
        ...
        "moveLead" => {
            ...
            match board::move_lead(db, &id, &to_stage, expected_version, actor) {
        ...
        "appendContext" => {
            ...
            let expected_version = get_i64("expectedVersion")?;
            match board::append_context(db, &id, research, expected_version, actor) {
        ...
        "draftMessage" => {
            ...
            match board::draft_message(db, &id, msg, actor) {
                Ok(seq) => Ok(json!({"ok": true, "data": {"seq": seq}})),
                Err(e) => Ok(json!({"ok": false, "error": board::board_err(e)})),
            }
        }
        "attachTranscript" => {
            ...
            match board::attach_transcript(db, &id, &raw, &summary, actor) {
                Ok(seq) => Ok(json!({"ok": true, "data": {"seq": seq}})),
                Err(e) => Ok(json!({"ok": false, "error": board::board_err(e)})),
            }
        }
        "renameStage" => {
            ...
            match board::rename_stage(db, &id, &label, actor) {
                Ok(version) => Ok(json!({"ok": true, "data": {"version": version}})),
                Err(e) => Ok(json!({"ok": false, "error": board::board_err(e)})),
            }
        }
        "reorderStages" => {
            ...
            match board::reorder_stages(db, &ids, actor) {
        ...
        "addStage" => {
            ...
            match board::add_stage(db, &label, position, actor) {
                Ok(id) => Ok(json!({"ok": true, "data": {"id": id}})),
                Err(e) => Ok(json!({"ok": false, "error": board::board_err(e)})),
            }
        }
        "retireStage" => {
            let id = get_str("id")?;
            match board::retire_stage(db, &id, actor) {
                Ok(seq) => Ok(json!({"ok": true, "data": {"seq": seq}})),
                Err(e) => Ok(json!({"ok": false, "error": board::board_err(e)})),
            }
        }
        "unretireStage" => {
            let id = get_str("id")?;
            match board::unretire_stage(db, &id, actor) {
                Ok(seq) => Ok(json!({"ok": true, "data": {"seq": seq}})),
                Err(e) => Ok(json!({"ok": false, "error": board::board_err(e)})),
            }
        }
        "remapStage" => {
            ...
            match board::remap_stage(db, &from, &to, filter, dry_run, retire_source, confirmed, actor) {
...
fn main() {
    ...
    match dispatch(&mut db, &actor.id, verb, &args) {
        Ok(response) => {
            println!("{response}");
        }
        Err(e) => {
...
mod tests {
    ...
    fn list_stages_returns_five_bootstrap_stages() {
        ...
        let resp = dispatch(&mut db, "local", "listStages", &json!({})).unwrap();
    ...
    fn add_lead_then_list_leads_shows_the_lead() {
        let (_tmp, mut db) = seeded_db();
        let add = dispatch(&mut db, "local", "addLead", &json!({"name": "Ana Costa", "org": "Acme", "stage": "researching"})).unwrap();
        assert_eq!(add["ok"], true);
        let id = add["data"]["id"].as_str().unwrap().to_string();

        let list = dispatch(&mut db, "local", "listLeads", &json!({})).unwrap();
    ...
    fn remap_stage_dry_run_reports_affected_and_writes_nothing() {
        let (_tmp, mut db) = seeded_db();
        for _ in 0..2 {
            dispatch(&mut db, "local", "addLead", &json!({"name": "X", "stage": "researching"})).unwrap();
        }
        let resp = dispatch(&mut db, "local", "remapStage", &json!({
