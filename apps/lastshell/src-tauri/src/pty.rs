...
//!
//! pty_open spawns $SHELL with cwd = the table's project dir and registers the
...
pub fn pty_open(
    ...
    if !project_path.is_dir() {
        return Err(format!("no such project dir: {project}"));
    }
    ...
    if shell_name == "zsh" {
        ...
        cmd.arg("-c");
        cmd.arg(format!("{agent_launch}exec {shell} -i"));
    }
